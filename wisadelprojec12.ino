// wis'adel waifu dev
// orang biasa yang kebetulan ada hoby eletronika dan juga suka dunia internet
...
void loop() {
  ...
  if (showGraph) {
    ...
    if (digitalRead(BTN_RESET) == LOW && isTripped) {
      resetProteksi();
    }
...
void checkProteksi() {
  bool outputState = (isTripped || manualProtActive);
  digitalWrite(activeProtPin, outputState ? HIGH : LOW);
  digitalWrite((activeProtPin == 12 ? 21 : 12), LOW); 


  bool phaseLossDetected = false; // ini bagian proteksi jika ada tegangan fasa yang hilang
  if (enablePhaseLoss) {
  ...
  if (phaseLossDetected && !isTripped) {
      isTripped = true;
      manualTripTriggered = false; // alaram pagat-pagat bunyinya
      tripStartTime = millis();
      manualProtActive = true;
  }
  ...
  bool vViolation = false; // bagian pembanding antara tegangan terbaca dengan tegangan max/min yang diset
  if (!isnan(cVR) && (cVR > limHighV || cVR < limLowV)) vViolation = true;
  if (!isnan(cVS) && (cVS > limHighV || cVS < limLowV)) vViolation = true;
  if (!isnan(cVT) && (cVT > limHighV || cVT < limLowV)) vViolation = true;

  bool currentNowOver = false; // bagian untuk membandingkan arus terbaca dngan maksimal arus nang diset
  if ((!isnan(cIR) && cIR > limMaxA) || (!isnan(cIS) && cIS > limMaxA) || (!isnan(cIT) && cIT > limMaxA)) {
    currentNowOver = true;
  }
  ...
  if (currentNowOver && !isTripped) {
    if (!isOvercurrent) {
      ...
      isOvercurrent = true;
    } else if (millis() - overcurrentStart > (limDelayA * 1000)) { // bagian untuk mencegah proteksi nyala kalo kena lonjakan dengan memberi waktu tunggu
        isTripped = true;
  ...
  bool pfLow = false; // buat pembatas minimal PF
  if (enablePFProt) {
     if ((!isnan(cPFR) && cPFR < limLowPF) || (!isnan(cPFS) && cPFS < limLowPF) || (!isnan(cPFT) && cPFT < limLowPF)) pfLow = true;
  }
  digitalWrite(LED_PF, pfLow ? HIGH : LOW); 
}

void handleEncoder() { // memfungsikan rotary encoder buat naik turun ketika memilih menu sekaligus untuk mengatur angka pada menu yang ada
    ...
    if (currentClk == LOW) {
      ...
      if (!editMode) {
        ...
        switch(menuIndex) {
          ...
          case 8: enablePhaseLoss = !enablePhaseLoss; break; // GANTI DISINI
  ...
  if (digitalRead(ENC_SW) == LOW) {
    if (millis() - lastButtonPress > 400) { 
      if (showGraph) {
        ...
        if (menuIndex == 9) { // GRAFIK ARUS
          inMenu = false;
          showGraph = true;
          tft.fillScreen(TFT_BLACK);
        } else if (menuIndex == 12) { // Restart
           resetProteksi();
           inMenu = false;
           tft.fillScreen(TFT_BLACK);
           drawUI();
        } else if (menuIndex == 13) { // SAVE EXIT
          saveSettings(); 
          inMenu = false;
          tft.fillScreen(TFT_BLACK);
          drawUI();
...
void updateMenuDisplay() {
  tft.setTextSize(2);
  String labels[] = { // ini yang akan muncul pada menu
    "Volt Max   ", "Volt Min   ", "Amp Max    ", "V/A Protec ", 
    "PF min set ", "PF Protec  ", "Cut Off    ", "Select Out ", "Loss Fasa  ", 
    "Grafik Arus", "Delay Protc", "Tampilan1/2", "Reset Protec", "Simpan keluar"
    // ini sebenarnya tesarah aja
  };
  for (int i = 0; i < 14; i++) {
    ...
    tft.print(labels[i]);
    //lappland waifu ku jua
    if (i != 9 && i != 12 && i != 13) tft.print(": ");

    switch(i) { // bagian keterangan pada pilihan menu
      ...
      case 8: tft.print(enablePhaseLoss ? "Aktif" : "kada Aktif"); break;
      case 10: tft.print(limDelayA, 0); break;
      case 11: tft.print(displayMode == 0 ? "Fasa-Netral" : "Fasa-Fasa"); break;
    }
    tft.print("      "); // pengalihan isu aja ini, hapus kd pp jua 
  }
}
void updateMonitoring() { //tampilan monitoring mode fasa ke netral
...
void updateRow(String phase, float v, float i, float w, float wh, float pf, int y, uint16_t phaseColor) {
  ...
  // ini biar kalo fasa ada nang hilang kedap kedip angkanya 
  ...
  tft.setTextColor(isTripped ? TFT_RED : TFT_MAGENTA, TFT_BLACK);
...
void updateMonitoringL2L() { // mode tampilan fasa ke fasa
  float vRS = sqrt(sq(cVR) + sq(cVS) + (cVR * cVS));
  float vST = sqrt(sq(cVS) + sq(cVT) + (cVS * cVT));
  float vTR = sqrt(sq(cVT) + sq(cVR) + (cVT * cVR));
...
void drawOscilloscope() { // ini tampilan grafik arus yang ada pada pilihan menu
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Wis'adel Project - Oscil Mode", 10, 5);
...
void drawUI() {
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.setTextSize(2);
  tft.drawString("Wis'adel project", 5, 0);
  ...
  tft.setCursor(220, 0); tft.setTextColor(TFT_PINK, TFT_BLACK);
...
void drawMenuBase() {
  ...
  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Wis'adel waifu Dev", 260, 302);
  updateMenuDisplay();
}
void loadSettings() { //ngambil data nang tesimpan di eprom
...
void saveSettings() { // ini untuk menyimpan ke eprom
...
float readDCVoltage() { //pembaca tegangan dc buat baca tegangan psu
  int raw = analogRead(ANALOG_DC_PIN);
  return (raw / 4095.0) * 3.3 * 11.0; 
}
void resetProteksi() { // tombol reset buat reset berbagai jenis keluhan dan masalah
  isTripped = false; manualTripTriggered = false; manualProtActive = false; isOvercurrent = false;
  digitalWrite(12, LOW); digitalWrite(21, LOW); digitalWrite(BUZZER, LOW);
}
void handleBuzzer() { // alaram
...
}// adel project meucap akan sekian dan tarimakasih